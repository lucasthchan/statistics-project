...
::: {style="font-size: 3em;"}
Is there an association between a member’s age and the number of services that they attended?
:::
...
I will attempt to use simple linear regression on the two variables, to find if an association exists between age and the number of services that a member attends. Finding this information allows Wellspring to discover if they should make changes to their services to cater to a more diverse age group. This linear regression will be *predictive* -- we are trying to see if a higher or lower age predicts lower attendance in service deliveries.
